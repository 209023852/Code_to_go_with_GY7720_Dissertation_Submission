...
Fire_Neither_iNat_df$datetime <- as.Date(Fire_Neither_iNat_df$datetime, format="%Y-%m-%d")
Fire_Neither_iNat_df$month <- as.vector(month(Fire_Neither_iNat_df$datetime))

### 3.1.1 Spatial Accuracy

Mean_SA_PreFire <- Fire_Neither_iNat_df %>% select(month, positional_accuracy) %>%
  filter(month < 9) %>%
  summarise(Mean_SA = mean(
    positional_accuracy, na.rm = T))

Mean_SA_Fire <- Fire_Neither_iNat_df %>% select(month, positional_accuracy) %>%
  filter(month == 9)%>%
  summarise(Mean_SA = mean(
    positional_accuracy, na.rm = T))

Mean_SA_PostFire <- Fire_Neither_iNat_df %>% select(month, positional_accuracy) %>%
  filter(month > 9)%>%
  summarise(Mean_SA = mean(
    positional_accuracy, na.rm = T))

Fire_Neither_iNat_SA <- data.frame(
...
Fire_Neither_iNat_df$Agreement_Score <- as.vector(Fire_Neither_iNat_df$num_identification_agreements-Fire_Neither_iNat_df$num_identification_disagreements)

Mean_FLA_PreFire <- Fire_Neither_iNat_df %>% select(month, Agreement_Score) %>%
  filter(month < 9) %>%
  summarise(Mean_FLA = mean(
    Agreement_Score, na.rm = T))

Mean_FLA_Fire <- Fire_Neither_iNat_df %>% select(month, Agreement_Score) %>%
  filter(month == 9)%>%
  summarise(Mean_FLA = mean(
    Agreement_Score, na.rm = T))

Mean_FLA_PostFire <- Fire_Neither_iNat_df %>% select(month, Agreement_Score) %>%
  filter(month > 9)%>%
  summarise(Mean_FLA = mean(
    Agreement_Score, na.rm = T))

Fire_Neither_iNat_FLA <- data.frame(
  Event_Period = c("iNat_Neither_PreFire", "iNat_Neither_Fire", "iNat_Neither_PostFire"), 
...
### 3.1.3 Completeness
Fire_Neither_iNat_Comp <- Fire_Neither_iNat_df %>%
  dplyr::group_by(month) %>%
  dplyr::count(month, quality_grade, sort=TRUE) %>%
  ungroup()

Fire_Neither_iNat_Comp <- Fire_Neither_iNat_Comp %>%
  dplyr::select(month, quality_grade, n) %>%
  dplyr::filter(quality_grade=="research") %>%
  dplyr::select(-quality_grade) %>%
  dplyr::arrange(month)
names(Fire_Neither_iNat_Comp)[2] <- "Comp_By_Month"

Mean_Comp_PreFire <- Fire_Neither_iNat_Comp %>% dplyr::select(month, Comp_By_Month) %>%
  dplyr::filter(month < 9) %>%
  dplyr::summarise(Mean_Comp = mean(
    Comp_By_Month, na.rm = T))

Mean_Comp_Fire <- Fire_Neither_iNat_Comp %>% select(month, Comp_By_Month) %>%
  filter(month == 9) %>%
  summarise(Mean_Comp = mean(
    Comp_By_Month, na.rm = T))

Mean_Comp_PostFire <- Fire_Neither_iNat_Comp %>%
  filter(month > 9) %>%
  summarise(Mean_Comp = mean(
    Comp_By_Month, na.rm = T))

Fire_Neither_iNat_Comp <- data.frame(
  Event_Period = c("iNat_Neither_PreFire", "iNat_Neither_Fire", "iNat_Neither_PostFire"), 
...
#Date Difference
Fire_Neither_iNat_df$DateDiff <- as.Date(as.character(ymd_hms(Fire_Neither_iNat_df$created_at)))-as.Date(as.character(Fire_Neither_iNat_df$datetime))

Mean_TA_PreFire <- Fire_Neither_iNat_df %>% select(month, DateDiff) %>%
  filter(month < 9) %>%
  summarise(Mean_TA = mean(
    DateDiff, na.rm = T))

Mean_TA_Fire <- Fire_Neither_iNat_df %>% select(month, DateDiff) %>%
  filter(month == 9) %>%
  summarise(Mean_TA = mean(
    DateDiff, na.rm = T))

Mean_TA_PostFire <- Fire_Neither_iNat_df %>% select(month, DateDiff) %>%
  filter(month > 9) %>%
  summarise(Mean_TA = mean(
    DateDiff, na.rm = T))

Fire_Neither_iNat_TA <- data.frame(
  Event_Period = c("iNat_Neither_PreFire", "iNat_Neither_Fire", "iNat_Neither_PostFire"), 
  Mean_TA = c(Mean_TA_PreFire[[1]],Mean_TA_Fire[[1]],Mean_TA_PostFire[[1]]))

Fire_Neither_iNat_TA <- transform(Fire_Neither_iNat_TA, Neither_Mean_TA = as.numeric(Neither_Mean_TA))

rm(Mean_TA_PreFire,Mean_TA_Fire,Mean_TA_PostFire)

### 3.1.5 Taxon Count
Fire_Neither_iNat_Taxon_Count <- Fire_Neither_iNat_df %>% select(id, month, iconic_taxon_name)
Fire_Neither_iNat_Taxon_Count$iconic_taxon_name[which(Fire_Neither_iNat_Taxon_Count$id == 52042724)] <- c("No_Name")
Fire_Neither_iNat_Taxon_Count$iconic_taxon_name[which(Fire_Neither_iNat_Taxon_Count$id == 47189040)] <- c("No_Name")
Fire_Neither_iNat_Taxon_Count$iconic_taxon_name[which(Fire_Neither_iNat_Taxon_Count$id == 43342333)] <- c("No_Name")
Fire_Neither_iNat_Taxon_Count$iconic_taxon_name[which(Fire_Neither_iNat_Taxon_Count$id == 43341442)] <- c("No_Name")
Fire_Neither_iNat_Taxon_Count <- Fire_Neither_iNat_Taxon_Count %>% pivot_wider(names_from = iconic_taxon_name, values_from = iconic_taxon_name, values_fn = list(iconic_taxon_name = ~1), values_fill = list(iconic_taxon_name = 0))
Fire_Neither_iNat_Taxon_Count <- Fire_Neither_iNat_Taxon_Count %>% select(-id)
Fire_Neither_iNat_Taxon_Count$Mollusca <- 0
Fire_Neither_iNat_Taxon_Count$Chromista <- 0
Fire_Neither_iNat_Taxon_Count$Amphibia <- 0
Fire_Neither_iNat_Taxon_Count$Actinopterygii <- 0
Fire_Neither_iNat_Taxon_Count$Protozoa <- 0

Fire_Neither_iNat_Taxon_Count <- Fire_Neither_iNat_Taxon_Count %>% select("month","Aves", "Plantae", "Fungi", "Animalia", "Insecta", "No_Name", "Arachnida", "Reptilia", "Mammalia", "Mollusca", "Chromista", "Amphibia", "Actinopterygii", "Protozoa") %>%
  group_by(month) %>%
  summarise(
...
Taxon_Count_PreFire <- Fire_Neither_iNat_Taxon_Count %>%
  filter(month < 9) %>%
  summarise(
...
Taxon_Count_Fire <- Fire_Neither_iNat_Taxon_Count %>%
  filter(month == 9) %>%
  summarise(
...
Taxon_Count_PostFire <- Fire_Neither_iNat_Taxon_Count %>%
  filter(month > 9) %>%
  summarise(
...
Fire_Neither_iNat_Taxon_Count <- data.frame(
  Event_Period = c("iNat_Neither_PreFire", "iNat_Neither_Fire", "iNat_Neither_PostFire"), 
...
Fire_Neither_iNat_Quality <- left_join(Fire_Neither_iNat_SA, Fire_Neither_iNat_FLA, by="Event_Period")
Fire_Neither_iNat_Quality <- left_join(Fire_Neither_iNat_Quality, Fire_Neither_iNat_Comp, by="Event_Period")
Fire_Neither_iNat_Quality <- left_join(Fire_Neither_iNat_Quality, Fire_Neither_iNat_TA, by="Event_Period")
Fire_Neither_iNat_Quality <- left_join(Fire_Neither_iNat_Quality, Fire_Neither_iNat_Taxon_Count, by="Event_Period")

rm(Fire_Neither_iNat_SA, Fire_Neither_iNat_FLA,Fire_Neither_iNat_Comp,Fire_Neither_iNat_TA,Fire_Neither_iNat_Taxon_Count)

Fire_Neither_iNat_Quality

write.csv(Fire_Neither_iNat_Quality, "Analysis/Quality_Loops/csv/Fire_Neither_iNat_Quality.csv")