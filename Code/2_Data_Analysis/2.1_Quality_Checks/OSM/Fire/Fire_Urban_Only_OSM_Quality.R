...
Fire_Urban_Only_OSM_df$month <- as.Date(as.character(ymd_hms(Fire_Urban_Only_OSM_df$X.validFrom)))
Fire_Urban_Only_OSM_df$month <- month(Fire_Urban_Only_OSM_df$month)

### 3.2.1 Spatial Accuracy
Fire_Urban_Only_OSM_SA <- Fire_Urban_Only_OSM_df %>%
  select(month, X.osmId, X.version) %>%
  group_by(month)

Fire_Urban_Only_OSM_SA_Urban_Only <-subset(Fire_Urban_Only_OSM_SA,ave(1:nrow(Fire_Urban_Only_OSM_SA),X.osmId,X.version, FUN = length)>1)
Fire_Urban_Only_OSM_SA_ID <-subset(Fire_Urban_Only_OSM_SA,ave(1:nrow(Fire_Urban_Only_OSM_SA),X.osmId, FUN = length)>1)
Fire_Urban_Only_OSM_SA_Anti <- anti_join(Fire_Urban_Only_OSM_SA_ID, Fire_Urban_Only_OSM_SA_Urban_Only, by=c("X.osmId","X.version"))

Fire_Urban_Only_OSM_SA <- Fire_Urban_Only_OSM_SA_Anti %>%
  group_by(month) %>%
  count(month, X.osmId, sort=TRUE)

rm(Fire_Urban_Only_OSM_SA_Urban_Only, Fire_Urban_Only_OSM_SA_ID, Fire_Urban_Only_OSM_SA_Anti)

Fire_Urban_Only_OSM_SA <- Fire_Urban_Only_OSM_SA %>%
  select(month, n) %>%
  group_by(month) %>%
  summarise(Frequency = sum(n))
names(Fire_Urban_Only_OSM_SA)[2] <- "Total_Changes_By_Month"

Mean_SA_PreFire <- Fire_Urban_Only_OSM_SA %>% select(month, Total_Changes_By_Month) %>%
  filter(month < 6) %>%
  summarise(Mean_SA = mean(
    Total_Changes_By_Month, na.rm = T))

Mean_SA_Fire <- Fire_Urban_Only_OSM_SA %>% select(month, Total_Changes_By_Month) %>%
  filter(month == 6)%>%
  summarise(Mean_SA = mean(
    Total_Changes_By_Month, na.rm = T))

Mean_SA_PostFire <- Fire_Urban_Only_OSM_SA %>% select(month, Total_Changes_By_Month) %>%
  filter(month > 6)%>%
  summarise(Mean_SA = mean(
    Total_Changes_By_Month, na.rm = T))

Fire_Urban_Only_OSM_SA <- data.frame(
...
Mean_FLA_PreFire <- Fire_Urban_Only_OSM_df %>% select(month, X.version) %>%
  filter(month < 6) %>%
  summarise(Mean_FLA = mean(
    X.version, na.rm = T))

Mean_FLA_Fire <- Fire_Urban_Only_OSM_df %>% select(month, X.version) %>%
  filter(month == 6)%>%
  summarise(Mean_FLA = mean(
    X.version, na.rm = T))

Mean_FLA_PostFire <- Fire_Urban_Only_OSM_df %>% select(month, X.version) %>%
  filter(month > 6)%>%
  summarise(Mean_FLA = mean(
    X.version, na.rm = T))

Fire_Urban_Only_OSM_FLA <- data.frame(
  Event_Period = c("OSM_Urban_Only_PreFire", "OSM_Urban_Only_Fire", "OSM_Urban_Only_PostFire"),  
...
### 3.2.3 Completeness
Fire_Urban_Only_OSM_df$NACount <- rowSums(is.na(Fire_Urban_Only_OSM_df))

Mean_Comp_PreFire <- Fire_Urban_Only_OSM_df %>% select(month, NACount) %>%
  filter(month < 6) %>%
  summarise(Mean_Comp = mean(
    NACount, na.rm = T))

Mean_Comp_Fire <- Fire_Urban_Only_OSM_df %>% select(month, NACount) %>%
  filter(month == 6)%>%
  summarise(Mean_Comp = mean(
    NACount, na.rm = T))

Mean_Comp_PostFire <- Fire_Urban_Only_OSM_df %>% select(month, NACount) %>%
  filter(month > 6)%>%
  summarise(Mean_Comp = mean(
    NACount, na.rm = T))

Fire_Urban_Only_OSM_Comp <- data.frame(
  Event_Period = c("OSM_Urban_Only_PreFire", "OSM_Urban_Only_Fire", "OSM_Urban_Only_PostFire"), 
...
#Date Difference
Fire_Urban_Only_OSM_df$DateDiff <- as.Date(as.character(ymd_hms(Fire_Urban_Only_OSM_df$X.validTo)))-as.Date(as.character(Fire_Urban_Only_OSM_df$X.validFrom))

Mean_TA_PreFire <- Fire_Urban_Only_OSM_df %>% select(month, DateDiff) %>%
  filter(month < 6) %>%
  summarise(Mean_TA = mean(
    DateDiff, na.rm = T))

Mean_TA_Fire <- Fire_Urban_Only_OSM_df %>% select(month, DateDiff) %>%
  filter(month == 6)%>%
  summarise(Mean_TA = mean(
    DateDiff, na.rm = T))

Mean_TA_PostFire <- Fire_Urban_Only_OSM_df %>% select(month, DateDiff) %>%
  filter(month > 6)%>%
  summarise(Mean_TA = mean(
    DateDiff, na.rm = T))

Fire_Urban_Only_OSM_TA <- data.frame(
  Event_Period = c("OSM_Urban_Only_PreFire", "OSM_Urban_Only_Fire", "OSM_Urban_Only_PostFire"),  
  Mean_TA = c(Mean_TA_PreFire[[1]],Mean_TA_Fire[[1]],Mean_TA_PostFire[[1]]))

Fire_Urban_Only_OSM_TA <- transform(Fire_Urban_Only_OSM_TA, Mean_TA = as.numeric(Mean_TA))

rm(Mean_TA_PreFire,Mean_TA_Fire,Mean_TA_PostFire)

### 3.2.4 Feature_Count

Fire_Urban_Only_OSM_Feature_Count <- Fire_Urban_Only_OSM_df

Fire_Urban_Only_OSM_Feature_Count$aeroway <- NA
Fire_Urban_Only_OSM_Feature_Count$historic <- NA
Fire_Urban_Only_OSM_Feature_Count$power <- NA
Fire_Urban_Only_OSM_Feature_Count$railway <- NA

Fire_Urban_Only_OSM_Feature_Count <- Fire_Urban_Only_OSM_Feature_Count %>%
...
Fire_Urban_Only_OSM_Feature_Count <- Fire_Urban_Only_OSM_Feature_Count %>% mutate_at(vars(-c(month)), ~replace(., !is.na(.), 1))
Fire_Urban_Only_OSM_Feature_Count[is.na(Fire_Urban_Only_OSM_Feature_Count)] <- 0

Fire_Urban_Only_OSM_Feature_Count <- Fire_Urban_Only_OSM_Feature_Count %>% mutate_if(is.character,as.numeric)
...
Feature_Count_PreFire <- Fire_Urban_Only_OSM_Feature_Count %>%
  filter(month < 6) %>%
  summarise(
...
Feature_Count_Fire <- Fire_Urban_Only_OSM_Feature_Count %>%
  filter(month == 6) %>%
  summarise(
...
Feature_Count_PostFire <- Fire_Urban_Only_OSM_Feature_Count %>%
  filter(month > 6) %>%
  summarise(
...
Fire_Urban_Only_OSM_Feature_Count <- data.frame(
  Event_Period = c("OSM_Urban_Only_PreFire", "OSM_Urban_Only_Fire", "OSM_Urban_Only_PostFire"), 
...
Fire_Urban_Only_OSM_Quality <- left_join(Fire_Urban_Only_OSM_SA, Fire_Urban_Only_OSM_FLA, by="Event_Period")
Fire_Urban_Only_OSM_Quality <- left_join(Fire_Urban_Only_OSM_Quality, Fire_Urban_Only_OSM_Comp, by="Event_Period")
Fire_Urban_Only_OSM_Quality <- left_join(Fire_Urban_Only_OSM_Quality, Fire_Urban_Only_OSM_TA, by="Event_Period")
Fire_Urban_Only_OSM_Quality <- left_join(Fire_Urban_Only_OSM_Quality, Fire_Urban_Only_OSM_Feature_Count, by="Event_Period")

rm(Fire_Urban_Only_OSM_SA, Fire_Urban_Only_OSM_FLA,Fire_Urban_Only_OSM_Comp,Fire_Urban_Only_OSM_TA,Fire_Urban_Only_OSM_Feature_Count)

Fire_Urban_Only_OSM_Quality

write.csv(Fire_Urban_Only_OSM_Quality, "Analysis/Quality_Loops/csv/Fire_Urban_Only_OSM_Quality.csv")