...
NonFire_Both_OSM_df$month <- as.Date(as.character(ymd_hms(NonFire_Both_OSM_df$X.validFrom)))
NonFire_Both_OSM_df$month <- month(NonFire_Both_OSM_df$month)

### 3.2.1 Spatial Accuracy
NonFire_Both_OSM_SA <- NonFire_Both_OSM_df %>%
  select(month, X.osmId, X.version) %>%
  group_by(month)

NonFire_Both_OSM_SA_Both <-subset(NonFire_Both_OSM_SA,ave(1:nrow(NonFire_Both_OSM_SA),X.osmId,X.version, FUN = length)>1)
NonFire_Both_OSM_SA_ID <-subset(NonFire_Both_OSM_SA,ave(1:nrow(NonFire_Both_OSM_SA),X.osmId, FUN = length)>1)
NonFire_Both_OSM_SA_Anti <- anti_join(NonFire_Both_OSM_SA_ID, NonFire_Both_OSM_SA_Both, by=c("X.osmId","X.version"))

NonFire_Both_OSM_SA <- NonFire_Both_OSM_SA_Anti %>%
  group_by(month) %>%
  count(month, X.osmId, sort=TRUE)

rm(NonFire_Both_OSM_SA_Both, NonFire_Both_OSM_SA_ID, NonFire_Both_OSM_SA_Anti)

NonFire_Both_OSM_SA <- NonFire_Both_OSM_SA %>%
  select(month, n) %>%
  group_by(month) %>%
  summarise(Frequency = sum(n))
names(NonFire_Both_OSM_SA)[2] <- "Total_Changes_By_Month"

Mean_SA_PreFire <- NonFire_Both_OSM_SA %>% select(month, Total_Changes_By_Month) %>%
  filter(month < 8) %>%
  summarise(Mean_SA = mean(
    Total_Changes_By_Month, na.rm = T))

Mean_SA_Fire <- NonFire_Both_OSM_SA %>% select(month, Total_Changes_By_Month) %>%
  filter(month == 8)%>%
  summarise(Mean_SA = mean(
    Total_Changes_By_Month, na.rm = T))

Mean_SA_PostFire <- NonFire_Both_OSM_SA %>% select(month, Total_Changes_By_Month) %>%
  filter(month > 8)%>%
  summarise(Mean_SA = mean(
    Total_Changes_By_Month, na.rm = T))

NonFire_Both_OSM_SA <- data.frame(
  Event_Period = c("OSM_Both_PreFire", "OSM_Both_Fire", "OSM_Both_PostFire"),  
...
Mean_FLA_PreFire <- NonFire_Both_OSM_df %>% select(month, X.version) %>%
  filter(month < 8) %>%
  summarise(Mean_FLA = mean(
    X.version, na.rm = T))

Mean_FLA_Fire <- NonFire_Both_OSM_df %>% select(month, X.version) %>%
  filter(month == 8)%>%
  summarise(Mean_FLA = mean(
    X.version, na.rm = T))

Mean_FLA_PostFire <- NonFire_Both_OSM_df %>% select(month, X.version) %>%
  filter(month > 8)%>%
  summarise(Mean_FLA = mean(
    X.version, na.rm = T))

NonFire_Both_OSM_FLA <- data.frame(
  Event_Period = c("OSM_Both_PreFire", "OSM_Both_Fire", "OSM_Both_PostFire"),  
...
### 3.2.3 Completeness
NonFire_Both_OSM_df$NACount <- rowSums(is.na(NonFire_Both_OSM_df))

Mean_Comp_PreFire <- NonFire_Both_OSM_df %>% select(month, NACount) %>%
  filter(month < 8) %>%
  summarise(Mean_Comp = mean(
    NACount, na.rm = T))

Mean_Comp_Fire <- NonFire_Both_OSM_df %>% select(month, NACount) %>%
  filter(month == 8)%>%
  summarise(Mean_Comp = mean(
    NACount, na.rm = T))

Mean_Comp_PostFire <- NonFire_Both_OSM_df %>% select(month, NACount) %>%
  filter(month > 8)%>%
  summarise(Mean_Comp = mean(
    NACount, na.rm = T))

NonFire_Both_OSM_Comp <- data.frame(
  Event_Period = c("OSM_Both_PreFire", "OSM_Both_Fire", "OSM_Both_PostFire"), 
...
#Date Difference
NonFire_Both_OSM_df$DateDiff <- as.Date(as.character(ymd_hms(NonFire_Both_OSM_df$X.validTo)))-as.Date(as.character(NonFire_Both_OSM_df$X.validFrom))

Mean_TA_PreFire <- NonFire_Both_OSM_df %>% select(month, DateDiff) %>%
  filter(month < 8) %>%
  summarise(Mean_TA = mean(
    DateDiff, na.rm = T))

Mean_TA_Fire <- NonFire_Both_OSM_df %>% select(month, DateDiff) %>%
  filter(month == 8)%>%
  summarise(Mean_TA = mean(
    DateDiff, na.rm = T))

Mean_TA_PostFire <- NonFire_Both_OSM_df %>% select(month, DateDiff) %>%
  filter(month > 8)%>%
  summarise(Mean_TA = mean(
    DateDiff, na.rm = T))

NonFire_Both_OSM_TA <- data.frame(
  Event_Period = c("OSM_Both_PreFire", "OSM_Both_Fire", "OSM_Both_PostFire"),  
  Mean_TA = c(Mean_TA_PreFire[[1]],Mean_TA_Fire[[1]],Mean_TA_PostFire[[1]]))

NonFire_Both_OSM_TA <- transform(NonFire_Both_OSM_TA, Mean_TA = as.numeric(Mean_TA))

rm(Mean_TA_PreFire,Mean_TA_Fire,Mean_TA_PostFire)

### 3.2.4 Feature_Count

NonFire_Both_OSM_Feature_Count <- NonFire_Both_OSM_df

#NonFire_Both_OSM_Feature_Count$emergency <- NA
#NonFire_Both_OSM_Feature_Count$office <- NA

NonFire_Both_OSM_Feature_Count <- NonFire_Both_OSM_Feature_Count %>%
...
NonFire_Both_OSM_Feature_Count <- NonFire_Both_OSM_Feature_Count %>% mutate_at(vars(-c(month)), ~replace(., !is.na(.), 1))
NonFire_Both_OSM_Feature_Count[is.na(NonFire_Both_OSM_Feature_Count)] <- 0

NonFire_Both_OSM_Feature_Count <- NonFire_Both_OSM_Feature_Count %>% mutate_if(is.character,as.numeric)
...
Feature_Count_PreFire <- NonFire_Both_OSM_Feature_Count %>%
  filter(month < 8) %>%
  summarise(
...
Feature_Count_Fire <- NonFire_Both_OSM_Feature_Count %>%
  filter(month == 8) %>%
  summarise(
...
Feature_Count_PostFire <- NonFire_Both_OSM_Feature_Count %>%
  filter(month > 8) %>%
  summarise(
...
NonFire_Both_OSM_Feature_Count <- data.frame(
  Event_Period = c("OSM_Both_PreFire", "OSM_Both_Fire", "OSM_Both_PostFire"), 
...
NonFire_Both_OSM_Quality <- left_join(NonFire_Both_OSM_SA, NonFire_Both_OSM_FLA, by="Event_Period")
NonFire_Both_OSM_Quality <- left_join(NonFire_Both_OSM_Quality, NonFire_Both_OSM_Comp, by="Event_Period")
NonFire_Both_OSM_Quality <- left_join(NonFire_Both_OSM_Quality, NonFire_Both_OSM_TA, by="Event_Period")
NonFire_Both_OSM_Quality <- left_join(NonFire_Both_OSM_Quality, NonFire_Both_OSM_Feature_Count, by="Event_Period")

rm(NonFire_Both_OSM_SA, NonFire_Both_OSM_FLA,NonFire_Both_OSM_Comp,NonFire_Both_OSM_TA,NonFire_Both_OSM_Feature_Count)

NonFire_Both_OSM_Quality

write.csv(NonFire_Both_OSM_Quality, "Analysis/Quality_Loops/csv/NonFire_Both_OSM_Quality.csv")