...
OSM_Cols <- c("@osmId","@version",NonFire_Neither_Feature_Type)
...
NonFire_Neither_OSM_shp <- NonFire_Neither_OSM_shp %>% mutate_at(vars(-c("@osmId","@version","DateDiff","NACount","month","geometry")), ~replace(., !is.na(.), 1))
...
Grid_NonFire_Neither <- 
  sf::st_read("data/Largest_NonFires/New_Grids/Grid_NonFire_Neither.shp")
# Make sure it is in WGS84
