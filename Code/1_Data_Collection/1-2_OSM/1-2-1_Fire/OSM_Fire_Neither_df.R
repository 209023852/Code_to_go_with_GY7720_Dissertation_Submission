...
df1 <- content(Fire_Neither_OSM_2019_1, "text")
# tidy the text
...
Fire_Neither_OSM_2019_1 <- data.frame(df1_type, df1_geom, df1_prop)
# Remove all temp files
rm(df1, df1_type,df1_geom,df1_prop)

# Repeat for other dates in 2019 and 2020 data

Fire_Neither_OSM_2019_2 = POST(
  "https://api.ohsome.org/v1/elementsFullHistory/geometry", 
  encode = "form", 
  body = list(
    bboxes = "-121.58667,36.39062,-121.71796,36.45139", 
...
df1 <- content(Fire_Neither_OSM_2019_2, "text")
df1 <- grep("^/\\* [0-9]* \\*/", df1, value = TRUE, invert = TRUE)
n1 <- length(df1)
df1[-n1] <- gsub("^}$", "},", df1[-n1])
df1 <- c("[", df1, "]")
...
df1_geom <- df1[[1]][["geometry"]]
df1_prop <- df1[[1]][["properties"]]
Fire_Neither_OSM_2019_2 <- data.frame(df1_type, df1_geom, df1_prop)
rm(df1, df1_type,df1_geom,df1_prop)

# Make 1 2019 file
Fire_Neither_OSM_2019 <- rbind.fill(Fire_Neither_OSM_2019_1,Fire_Neither_OSM_2019_2)
# Remove temp files
rm(Fire_Neither_OSM_2019_1,Fire_Neither_OSM_2019_2)
...
#rm(Fire_Neither_Bounds_OSM)

df1 <- content(Fire_Neither_OSM_list[[1]], "text")
df1 <- grep("^/\\* [0-9]* \\*/", df1, value = TRUE, invert = TRUE)
n1 <- length(df1)
df1[-n1] <- gsub("^}$", "},", df1[-n1])
df1 <- c("[", df1, "]")
...
# remove 2019 data
df1 <- anti_join(df1, Fire_Neither_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df1_type,df1_geom,df1_prop)

df2 <- content(Fire_Neither_OSM_list[[2]], "text")
...
df2 <- anti_join(df2, Fire_Neither_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df2_type,df2_geom,df2_prop)

df3 <- content(Fire_Neither_OSM_list[[3]], "text")
...
df3 <- anti_join(df3, Fire_Neither_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df3_type,df3_geom,df3_prop)

df4 <- content(Fire_Neither_OSM_list[[4]], "text")
...
df4 <- anti_join(df4, Fire_Neither_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df4_type,df4_geom,df4_prop)

df5 <- content(Fire_Neither_OSM_list[[5]], "text")
...
df5 <- anti_join(df5, Fire_Neither_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df5_type,df5_geom,df5_prop)

df6 <- content(Fire_Neither_OSM_list[[6]], "text")
...
df6 <- anti_join(df6, Fire_Neither_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df6_type,df6_geom,df6_prop)

df7 <- content(Fire_Neither_OSM_list[[7]], "text")
...
df7 <- anti_join(df7, Fire_Neither_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df7_type,df7_geom,df7_prop)

df8 <- content(Fire_Neither_OSM_list[[8]], "text")
...
df8 <- anti_join(df8, Fire_Neither_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df8_type,df8_geom,df8_prop)

df9 <- content(Fire_Neither_OSM_list[[9]], "text")
...
df9 <- anti_join(df9, Fire_Neither_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df9_type,df9_geom,df9_prop)

df10 <- content(Fire_Neither_OSM_list[[10]], "text")
...
df10 <- anti_join(df10, Fire_Neither_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df10_type,df10_geom,df10_prop)

df11 <- content(Fire_Neither_OSM_list[[11]], "text")
...
df11 <- anti_join(df11, Fire_Neither_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df11_type,df11_geom,df11_prop)

df12 <- content(Fire_Neither_OSM_list[[12]], "text")
...
df12 <- anti_join(df12, Fire_Neither_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df12_type,df12_geom,df12_prop)

# make a singular 2020 file
Fire_Neither_OSM_df <- rbind.fill(df1,df2,df3,df4,df5,df6,df7,df8,df9,df10,df11,df12)
...
#write.csv(Fire_Neither_OSM_df,"data/csv/Fire_Neither_OSM_df.csv")
