...
write.csv(Fire_CPAD_Only_iNat_shp,"data/shp/iNat/Fire/csv/Fire_CPAD_Only_iNat_shp.csv")
...
#mapview(Fire_CPAD_Only_iNat_shp, map.type="OpenStreetMap")
#mapview(Grid_Fire_CPAD_Only, map.type="OpenStreetMap")

# All
iNat_AllFire_CPAD_Only_Grids <- sf::st_join(Grid_Fire_CPAD_Only,Fire_CPAD_Only_iNat_shp)

iNat_AllFire_CPAD_Only_Grids <- iNat_AllFire_CPAD_Only_Grids %>% select("id.x", "positional_accuracy", "quality_grade", "Agreement_Score", "DateDiff", "Aves", "Plantae", "Fungi", "Animalia", "Insecta", "No_Name", "Arachnida", "Reptilia", "Mammalia", "Mollusca", "Chromista", "Amphibia", "Actinopterygii", "Protozoa") %>%
  group_by(id.x) %>%
  summarise(
...
iNat_AllFire_CPAD_Only_Grids[is.na(iNat_AllFire_CPAD_Only_Grids)] <- 0
