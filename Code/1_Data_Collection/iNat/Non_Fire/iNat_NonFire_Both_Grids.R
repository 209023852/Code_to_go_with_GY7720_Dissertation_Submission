...
write.csv(NonFire_Both_iNat_shp,"data/shp/iNat/NonFire/csv/NonFire_Both_iNat_shp.csv")
...
Grid_NonFire_Both <- 
  sf::st_read("data/Largest_Fires/New_Grids/Grid_NonFire_Both.shp")
Grid_NonFire_Both <- st_transform(Grid_NonFire_Both, 4326)
Grid_NonFire_Both$id <- seq.int(nrow(Grid_NonFire_Both))
NonFire_Both_iNat_shp <- NonFire_Both_iNat_shp[Grid_NonFire_Both, ]
NonFire_Both_iNat_shp[is.na(NonFire_Both_iNat_shp)] <- 0
st_write(NonFire_Both_iNat_shp, "data/shp/iNat/NonFire/NonFire_Both_iNat_shp.shp")
#mapview(NonFire_Both_iNat_shp, map.type="OpenStreetMap")
#mapview(Grid_NonFire_Both, map.type="OpenStreetMap")

# All
iNat_AllNonFire_Both_Grids <- sf::st_join(Grid_NonFire_Both,NonFire_Both_iNat_shp)

iNat_AllNonFire_Both_Grids <- iNat_AllNonFire_Both_Grids %>% select("id.x", "positional_accuracy", "quality_grade", "Agreement_Score", "DateDiff", "Aves", "Plantae", "Fungi", "Animalia", "Insecta", "No_Name", "Arachnida", "Reptilia", "Mammalia", "Mollusca", "Chromista", "Amphibia", "Actinopterygii", "Protozoa") %>%
  group_by(id.x) %>%
  summarise(
...
iNat_AllNonFire_Both_Grids[is.na(iNat_AllNonFire_Both_Grids)] <- 0
