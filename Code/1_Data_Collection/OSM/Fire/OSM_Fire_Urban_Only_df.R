...
Fire_Urban_Only_OSM_2019 <- data.frame(df1_type, df1_geom, df1_prop)
...
#Fire_Urban_Only_Bounds_OSM <- Fire_Urban_Only[1,] %>%
#  sf::st_bbox()
#Fire_Urban_Only_Bounds_OSM <- c(Fire_Urban_Only_Bounds_OSM[[3]],
#                                Fire_Urban_Only_Bounds_OSM[[2]],
 #                               Fire_Urban_Only_Bounds_OSM[[1]],
#                                Fire_Urban_Only_Bounds_OSM[[4]])
#
#Fire_Urban_Only_Bounds_OSM

Fire_Urban_Only_OSM_list = list()
for (i in 1:12){
  tryCatch({
    temp = POST(
      "https://api.ohsome.org/v1/elementsFullHistory/geometry", 
      ...
      body = list(
        ...
        time = osm_month[[i]], 
        showMetadata = "yes", properties = "metadata,tags"))
    Fire_Urban_Only_OSM_list[[i]] = temp
  }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}

#rm(Fire_Urban_Only_Bounds_OSM)

df1 <- content(Fire_Urban_Only_OSM_list[[1]], "text")
df1 <- grep("^/\\* [0-9]* \\*/", df1, value = TRUE, invert = TRUE)
n1 <- length(df1)
df1[-n1] <- gsub("^}$", "},", df1[-n1])
df1 <- c("[", df1, "]")
...
df1 <- anti_join(df1, Fire_Urban_Only_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df1_type,df1_geom,df1_prop)

df2 <- content(Fire_Urban_Only_OSM_list[[2]], "text")
...
df2 <- anti_join(df2, Fire_Urban_Only_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df2_type,df2_geom,df2_prop)

df3 <- content(Fire_Urban_Only_OSM_list[[3]], "text")
...
df3 <- anti_join(df3, Fire_Urban_Only_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df3_type,df3_geom,df3_prop)

df4 <- content(Fire_Urban_Only_OSM_list[[4]], "text")
...
df4 <- anti_join(df4, Fire_Urban_Only_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df4_type,df4_geom,df4_prop)

df5 <- content(Fire_Urban_Only_OSM_list[[5]], "text")
...
df5 <- anti_join(df5, Fire_Urban_Only_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df5_type,df5_geom,df5_prop)

df6 <- content(Fire_Urban_Only_OSM_list[[6]], "text")
...
df6 <- anti_join(df6, Fire_Urban_Only_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df6_type,df6_geom,df6_prop)

df7 <- content(Fire_Urban_Only_OSM_list[[7]], "text")
...
df7 <- anti_join(df7, Fire_Urban_Only_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df7_type,df7_geom,df7_prop)

df8 <- content(Fire_Urban_Only_OSM_list[[8]], "text")
...
df8 <- anti_join(df8, Fire_Urban_Only_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df8_type,df8_geom,df8_prop)

df9 <- content(Fire_Urban_Only_OSM_list[[9]], "text")
...
df9 <- anti_join(df9, Fire_Urban_Only_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df9_type,df9_geom,df9_prop)

df10 <- content(Fire_Urban_Only_OSM_list[[10]], "text")
...
df10 <- anti_join(df10, Fire_Urban_Only_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df10_type,df10_geom,df10_prop)

df11 <- content(Fire_Urban_Only_OSM_list[[11]], "text")
...
df11 <- anti_join(df11, Fire_Urban_Only_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df11_type,df11_geom,df11_prop)

df12 <- content(Fire_Urban_Only_OSM_list[[12]], "text")
...
df12 <- anti_join(df12, Fire_Urban_Only_OSM_2019, by = c("X.osmId" = "X.osmId", "X.version" = "X.version"))
rm(df12_type,df12_geom,df12_prop)

Fire_Urban_Only_OSM_df <- rbind.fill(df1,df2,df3,df4,df5,df6,df7,df8,df9,df10,df11,df12)

rm(Fire_Urban_Only_OSM_2019,Fire_Urban_Only_OSM_list,df1,n1,df2,n2,df3,n3,df4,n4,df5,n5,df6,n6,df7,n7,df8,n8,df9,n9,df10,n10,df11,n11,df12,n12)
#write.csv(Fire_Urban_Only_OSM_df,"data/csv/Fire_Urban_Only_OSM_df.csv")
