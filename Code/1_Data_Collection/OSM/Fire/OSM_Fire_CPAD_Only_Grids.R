...
OSM_Cols <- c("X.osmId","X.version",Fire_CPAD_Only_Feature_Type)
...
Fire_CPAD_Only_OSM_shp <- Fire_CPAD_Only_OSM_shp %>% mutate_at(vars(-c("X.osmId","X.version","DateDiff","NACount","month","geometry")), ~replace(., !is.na(.), 1))
...
Grid_Fire_CPAD_Only$id <- seq.int(nrow(Grid_Fire_CPAD_Only))
