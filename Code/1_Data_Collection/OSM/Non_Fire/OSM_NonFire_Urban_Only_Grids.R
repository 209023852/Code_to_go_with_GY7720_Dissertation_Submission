...
OSM_Cols <- c("X.osmId","X.version",NonFire_Urban_Only_Feature_Type)
...
NonFire_Urban_Only_OSM_shp <- NonFire_Urban_Only_OSM_shp %>% mutate_at(vars(-c("X.osmId","X.version","DateDiff","NACount","month","geometry")), ~replace(., !is.na(.), 1))
...
Invalid_Rows_Pre_NonFire_Urban_Only_OSM <- Pre_NonFire[c(458,462,2984,7131,10124)]
Pre_NonFire <- Pre_NonFire[-c(458,462,2984,7131,10124),] 
...
Invalid_Rows_Pre_NonFire_Urban_Only_OSM <- NonFire[c(1514)]
NonFire <- NonFire[-c(1514),] 
...
Invalid_Rows_Pre_Post_NonFire_Urban_Only_OSM <- Post_NonFire[c(244)]
Post_NonFire <- Post_NonFire[-c(244),] 
