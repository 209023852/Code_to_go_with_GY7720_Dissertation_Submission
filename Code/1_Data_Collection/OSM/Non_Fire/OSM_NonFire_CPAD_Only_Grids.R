...
OSM_Cols <- c("X.osmId","X.version",NonFire_CPAD_Only_Feature_Type)
...
NonFire_CPAD_Only_OSM_shp <- NonFire_CPAD_Only_OSM_shp %>% mutate_at(vars(-c("X.osmId","X.version","DateDiff","NACount","month","geometry")), ~replace(., !is.na(.), 1))
...
Grid_NonFire_CPAD_Only <- 
  sf::st_read("data/Largest_Fires/New_Grids/Grid_NonFire_CPAD_Only.shp")
